...
var user = {
    ...
    ,addUclass : function(id){
        id = parseInt(id)<1?0:parseInt(id);
        location.href = cscms_path+'index.php/vod/user/uclass/edit/'+id;
        /*var title = (id==0)?'添加分类':'修改分类';
        cscms.layer.prompt({
            formType: 2,
            value: value,
            title: title,
            ...
            if(value==''){
                cscms.layer.msg('分类名称不能为空',{icon:2});return;
            }else{
                $.post(cscms_path+'index.php/vod/user/uclass/save', {
                    id:id,name:value
                }, function(data) {
                    if(data.error==0){
                        cscms.layer.msg('恭喜您，操作成功',{icon:1});
                        if(id==0){
                            location.reload();
                        }else{
                            $('#uname_'+id).html(value);
                            $('#umdf_'+id).attr('href', 'javascript:user.addUclass('+id+',\''+value+'\');');
                        }
                        layer.close(index);  
                    }else{
                        cscms.layer.msg(data.msg,{icon:2});
    ...
    ,uv_del : function(id){
        if(id<1){
            cscms.layer.msg('参数不正确，请刷新重试',{icon:2});return;
        }
        cscms.layer.confirm('确定要删除该分类吗？', {
            btn: ['确定', '取消'] //按钮
        }, function(index) {
            $.post(cscms_path+'index.php/vod/user/uclass/del', {
                id:id
            }, function(data) {
                if(data.error==0){
                    cscms.layer.msg('恭喜您，删除成功',{icon:1});
